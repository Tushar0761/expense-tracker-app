  ...
  return (
    <div className="container mx-auto py-6 space-y-6 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Accounts & Transfers</h1>
          <p className="text-[13px] text-muted-foreground">Manage your money across different accounts and track movements.</p>
        </div>
          ...
          <Dialog open={isTransferDialogOpen} onOpenChange={setIsTransferDialogOpen}>
            ...
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                ...
                <DialogDescription className="text-xs text-muted-foreground">Move funds between your accounts.</DialogDescription>
              </DialogHeader>
              <form onSubmit={handleCreateTransfer} className="space-y-4 py-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Select name="fromAccountId" required>
                    ...
                    <SelectContent>
                      {accounts.map(acc => (
                        <SelectItem key={acc.id} value={acc.id.toString()}>{acc.name} (₹{acc.balance.toLocaleString()})</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                ...
                <DialogFooter>
                  <Button type="submit" className="w-full" disabled={createTransferMutation.isPending}>
                    {createTransferMutation.isPending ? "Recording..." : "Execute Transfer"}
                  </Button>
                </DialogFooter>
              </form>
            </DialogContent>
          ...
          <Dialog open={isAccountDialogOpen} onOpenChange={setIsAccountDialogOpen}>
            ...
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                ...
                <DialogDescription className="text-xs text-muted-foreground">Create a new account to track your finances.</DialogDescription>
              </DialogHeader>
              <form onSubmit={handleCreateAccount} className="space-y-4 py-4">
                <div className="grid gap-2">
                  <Label htmlFor="name">Account Name</Label>
                  <Input id="name" name="name" placeholder="e.g. HDFC Bank, My Wallet" required />
                </div>
                <div className="grid gap-2">
                ...
                <div className="grid gap-2">
                  <Label htmlFor="creditLimit">Credit Limit (₹) - Only for Credit Cards</Label>
                  <Input id="creditLimit" name="creditLimit" type="number" step="0.01" defaultValue="0" />
                </div>
                <DialogFooter>
        ...
        <CardContent className="py-4 px-6">
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Total Net Liquidity</p>
              <h2 className={`text-2xl font-bold ${totalBalance >= 0 ? "text-emerald-500" : "text-rose-500"} tracking-tight`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="flex items-center gap-1.5 px-1 pt-1">
            ...
            <h3 className="text-sm font-bold uppercase tracking-tight text-muted-foreground">Your Accounts</h3>
          </div>
          <div className="grid gap-3">
            {isAccountsLoading ? (
              [1, 2, 3].map(i => <Card key={i} className="h-16 animate-pulse bg-muted" />)
            ) : accounts.length === 0 ? (
              <p className="text-center text-xs text-muted-foreground py-6">No accounts found. Add one to get started.</p>
            ) : (
              accounts.map((acc: Account) => (
                <Card key={acc.id} className="group hover:border-primary/40 transition-all duration-300 overflow-hidden relative bg-card/50">
                   {acc.type === 'CREDIT' && (
                  ...
                  <CardContent className="p-3 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs font-semibold group-hover:text-primary transition-colors">{acc.name}</p>
        ...
        <div className="lg:col-span-2 space-y-3">
          <div className="flex items-center gap-1.5 px-1 pt-1">
            ...
            <h3 className="text-sm font-bold uppercase tracking-tight text-muted-foreground">Transfer History</h3>
          </div>
          <Card className="border border-border/50 bg-card/20 overflow-hidden">
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/30">
                  <TableHead className="h-8 text-[11px] uppercase font-bold py-2">Date</TableHead>
                  <TableHead className="h-8 text-[11px] uppercase font-bold py-2">From</TableHead>
                  <TableHead className="h-8 text-[11px] uppercase font-bold py-2"></TableHead>
                  <TableHead className="h-8 text-[11px] uppercase font-bold py-2">To</TableHead>
                  <TableHead className="h-8 text-[11px] uppercase font-bold py-2 text-right">Amount</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {isTransfersLoading ? (
                  <TableRow><TableCell colSpan={5} className="text-center py-8">Loading transfers...</TableCell></TableRow>
                ) : transfers.length === 0 ? (
                  <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground">No transfers recorded.</TableCell></TableRow>
                ) : (
                  transfers.map((t) => (
                    <TableRow key={t.id} className="hover:bg-primary/[0.02] transition-colors border-b border-border/10">
                      <TableCell className="py-2 text-[12px] tabular-nums text-muted-foreground">{format(new Date(t.date), "dd MMM yy")}</TableCell>
                      <TableCell className="py-2">
                        <div className="flex items-center gap-1.5">
                           {t.fromAccount && <div className="h-4 w-4">{getAccountIcon(t.fromAccount.type)}</div>}
                           <span className="text-[12px] font-medium">{t.fromAccount?.name || "Deleted"}</span>
                        </div>
                      </TableCell>
                      <TableCell className="py-2">
                      ...
                      <TableCell className="py-2">
                         <div className="flex items-center gap-1.5">
                           {t.toAccount && <div className="h-4 w-4">{getAccountIcon(t.toAccount.type)}</div>}
                           <span className="text-[12px] font-medium">{t.toAccount?.name || "Deleted"}</span>
                        </div>
                      </TableCell>
                      <TableCell className="py-2 text-right font-bold text-blue-500 tabular-nums text-[12px]">₹{t.amount.toLocaleString()}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
