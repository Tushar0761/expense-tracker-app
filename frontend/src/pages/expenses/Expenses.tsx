                ...
                <Card className="p-3 bg-muted/20 border-border/40">
                    <div className="flex gap-4 flex-wrap items-center">
                        <div className="grid gap-1">
                            <span className="text-[9px] uppercase font-bold text-muted-foreground ml-0.5">Range</span>
                        ...
                        <div className="grid gap-1">
                            <span className="text-[9px] uppercase font-bold text-muted-foreground ml-0.5">Category</span>
                            <select
                        ...
                        <div className="grid gap-1 flex-1">
                            <span className="text-[9px] uppercase font-bold text-muted-foreground ml-0.5">Search</span>
                            <input
                                type="text"
                                value={search}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="bg-card/30">
                    <CardHeader className="p-3 pb-1 flex flex-row items-center justify-between">
                        <CardTitle className="text-[11px] font-bold text-muted-foreground uppercase">Total (Page)</CardTitle>
                    ...
                    <CardContent className="p-3 pt-0">
                        <div className="text-xl font-bold text-rose-600 tracking-tight">₹{kpis.totalExpense.toLocaleString()}</div>
                        <p className="text-[10px] text-muted-foreground mt-0.5">{kpis.numTransactions} transactions found</p>
                    </CardContent>
                </Card>
                ...
                <Card className="md:col-span-2 bg-card/30">
                    <CardHeader className="p-3 pb-1">
                        <div className="flex items-center justify-between">
                            <CardTitle className="text-[11px] font-bold text-muted-foreground uppercase">Spending by Category</CardTitle>
                        ...
                        <div className="flex gap-3 flex-wrap">
                            {kpis.topCategories.length > 0 ? (
                                kpis.topCategories.map((cat) => (
                                    <div key={cat.name} className="flex-1 min-w-[100px] bg-muted/20 px-2.5 py-1.5 rounded-lg border border-border/30">
                                        <p className="text-[9px] uppercase font-bold text-muted-foreground leading-tight">{cat.name}</p>
                                        <p className="text-sm font-bold tracking-tight">₹{cat.value.toLocaleString()}</p>
                                    </div>
                                ))
                            ) : (
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y">
                                {isLoading ? (
                                    <tr><td colSpan={6} className="py-12 text-center text-muted-foreground animate-pulse">Loading amazing data...</td></tr>
                                ) : expenses.length === 0 ? (
                                    <tr><td colSpan={6} className="py-12 text-center text-muted-foreground">No expenses found matching your criteria.</td></tr>
                                ) : (
                                    expenses.map((tx) => (
                                        <tr
                                            key={tx.id}
