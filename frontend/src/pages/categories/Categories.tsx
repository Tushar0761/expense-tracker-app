        ...
        <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">Manage your expense types and subcategories</p>
                </div>
            ...
            <Card className="border-none shadow-premium bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm">
                <CardContent className="p-6">
                    <div className="space-y-4">
                        {categories.map((cat) => (
                            <div key={cat.id} className="border rounded-lg overflow-hidden border-gray-100 dark:border-gray-800">
                                <div className="flex items-center justify-between p-4 bg-gray-50/50 dark:bg-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Button 
                                            ...
                                            onClick={() => {
                                                if (confirm(`Delete category "${cat.name}" and all subcategories?`)) {
                                ...
                                {expandedCats.has(cat.id) && (
                                    <div className="p-2 space-y-1 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 animate-in slide-in-from-top-2 duration-200">
                                        {cat.subCategories.length === 0 ? (
                                            <p className="text-sm text-muted-foreground p-4 text-center">No subcategories yet.</p>
                                        ) : (
                        ...
                        {categories.length === 0 && (
                            <div className="text-center py-12">
                                <p className="text-muted-foreground mb-4">You haven't added any categories yet.</p>
                                <Button onClick={() => openAddDialog()}>Create your first category</Button>
                            </div>
                        )}
                    </div>
                </CardContent>
            ...
            <Dialog open={isAddCatOpen} onOpenChange={setIsAddCatOpen}>
                <DialogContent>
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setIsAddCatOpen(false)}>Cancel</Button>
                        <Button onClick={handleCreate} disabled={createMutation.isPending || !newCatName.trim()}>
