...
export function Dashboard() {
    ...
    const monthChange = useMemo(() => {
        ...
        return (
            ((kpis.thisMonth.total - kpis.lastMonth.total) /
                kpis.lastMonth.total) *
            100
        );
    }, [kpis]);
    ...
    const totalBankBalance = useMemo(() => {
        ...
        return kpis.accounts.reduce((sum, acc) => 
    ...
    const getAccountIcon = (type: string) => {
        switch (type) {
            ...
            case "CREDIT": return <CreditCard className="h-4 w-4 text-rose-500" />;
        ...
        <div className="space-y-6 animate-in fade-in duration-700">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
               <div>
                  ...
                  <p className="text-[13px] text-muted-foreground">Keep track of your spending and balances at a glance.</p>
               </div>
               <div className="bg-primary/5 border border-primary/10 rounded-xl px-4 py-2 flex gap-3 items-center">
                  <div className="text-right">
                     <p className="text-[9px] uppercase font-bold text-muted-foreground tracking-widest">Net Liquidity</p>
                     <p className={`text-lg font-black ${totalBankBalance >= 0 ? "text-primary" : "text-rose-600"}`}>
                ...
                {(kpis?.accounts ?? []).map(acc => (
                    <Card key={acc.id} className="overflow-hidden group hover:border-primary/40 transition-all bg-card/40">
                        <CardHeader className="p-3 pb-1.5 space-y-0 flex flex-row items-center justify-between">
                            <span className="text-[9px] uppercase font-bold text-muted-foreground">{acc.type}</span>
                            {getAccountIcon(acc.type)}
                        </CardHeader>
                        <CardContent className="p-3 pt-0">
                            <h3 className="text-xs font-semibold truncate mb-0.5">{acc.name}</h3>
                            <p className={`text-base font-bold ${acc.type === 'CREDIT' ? 'text-rose-500' : 'text-emerald-500'}`}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <KpiCard
                    ...
                    description={monthChange > 0 ? "Increased spending" : "Decreased spending"}
                    Icon={<TrendingDown size={16} className={monthChange > 0 ? "text-rose-500" : "text-emerald-500"} />}
                        ...
                        {barData.length > 0 ? (
                            <ResponsiveContainer width="100%" height={220}>
                                <BarChart data={barData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                                    <XAxis dataKey="month" axisLine={false} tickLine={false} fontSize={10} tick={{ fill: 'var(--muted-foreground)' }} />
                                    <YAxis
                                        axisLine={false}
                                        tickLine={false}
                                        fontSize={10}
                            ...
                            <div className="text-xs text-muted-foreground py-8 text-center">No data found.</div>
                        )}
                    </CardContent>
                </Card>
                            ...
                            <div className="flex flex-col items-center">
                                <ResponsiveContainer width="100%" height={160}>
                                    <PieChart>
                                        ...
                                        <Tooltip 
                                            contentStyle={{ borderRadius: '8px', border: '1px solid var(--border)', fontSize: '11px' }}
                                ...
                                <div className="grid grid-cols-2 gap-x-3 gap-y-1 w-full mt-2">
                                   {pieData.slice(0, 4).map((entry, index) => (
                                       <div key={entry.name} className="flex items-center gap-1.5">
                                          ...
                                          <span className="text-[10px] truncate leading-tight text-muted-foreground">{entry.name}</span>
                                       </div>
                                   ))}
                                </div>
                            </div>
                        ) : (
                            <div className="text-xs text-muted-foreground py-8 text-center">No category data.</div>
                        )}
                    </CardContent>
                </Card>
            </div>
                            ...
                            <tbody className="divide-y divide-border/20">
                                {(kpis?.recentTransactions ?? []).map((tx) => (
                                    <tr key={tx.id} className="hover:bg-muted/30 transition-colors">
                                        ...
                                        <td className="py-2 px-4">
                                            <div className="flex gap-1 flex-wrap">
                                                {tx.categories.map((c, i) => (
                                                    <Badge key={i} variant="secondary" className="text-[9px] py-0 px-1 h-3.5 font-normal">
                                                        {c}
                                                    </Badge>
                                                ))}
                                        ...
                                        <td className="py-2 px-4 truncate max-w-[150px] text-xs">{tx.remarks || "-"}</td>
