                  ...
                  {tableData.map((loan, i) => (
                    <tr key={loan.id} className="border-b last:border-0">
                      ...
                      <td className="py-2 px-4 whitespace-nowrap">
                        {format(parseISO(loan.loanDate), 'dd-MMM yyy')}
                      </td>
                      <td className="py-2 px-4 whitespace-nowrap">
                        {loan.borrowerName}
                      </td>
                      <td className="py-2 px-4 whitespace-nowrap font-bold">
                        {loan.totalAmount.toLocaleString('hi-IN')} ₹
                      </td>

                      <td className="py-2 px-4 whitespace-nowrap">
                        {loan.paidAmount.toLocaleString('hi-IN') ?? 0} ₹
                      </td>
                      <td className="py-2 px-4 whitespace-nowrap">
                        {loan.remainingAmount.toLocaleString('hi-IN') ?? 0} ₹
                      </td>

                      <td className="py-2 px-4 whitespace-nowrap align-top">
                        {loan.notes ? (
                          loan.notes.split(';').length > 1 ? (
                            <ul className="list-disc list-inside space-y-1">
                              {loan.notes.split(';').map((noteRaw, idx) => {
                                const [text, amount] = noteRaw
                                  .trim()
                                  .split('--');
                                return (
                                  <li
