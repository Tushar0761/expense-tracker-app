...
export const SearchableSelect = React.forwardRef<HTMLDivElement, SearchableSelectProps>(
    ({ value, onChange, options, placeholder = "Select...", error, className }, ref) => {
        ...
        const filteredOptions = React.useMemo(() => {
            ...
            return options.filter(opt => 
                ...
                (opt.parentName && opt.parentName.toLowerCase().includes(searchLower))
            );
        }, [options, search]);
                    ...
                    {selectedOption ? (
                        <span className="flex flex-col items-start">
                            ...
                            {selectedOption.parentName && (
                                <span className="text-xs text-muted-foreground font-normal">{selectedOption.parentName}</span>
                            )}
                        </span>
                    ) : (
                        <span className="text-muted-foreground">{placeholder}</span>
                    )}
                    <ChevronDown className={cn("h-4 w-4 opacity-50 transition-transform shrink-0 ml-2", isOpen && "rotate-180")} />
                </button>

                {isOpen && (
                    <div className="absolute z-50 mt-1 w-full rounded-md border bg-popover p-0 shadow-lg outline-none animate-in fade-in-0 zoom-in-95">
                                ...
                                filteredOptions.map((option) => (
                                    ...
                                    >
                                        <div className="flex flex-col items-start flex-1">
                                            <span className={cn("font-medium", option.id === value && "text-primary")}>
                                                {option.name}
                                            </span>
                                            {option.parentName && (
                                                <span className="text-xs text-muted-foreground">{option.parentName}</span>
                                            )}
                                        </div>
                                        {option.id === value && (
